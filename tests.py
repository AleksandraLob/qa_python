...
class TestBooksCollector:
    ...
    def test_add_new_book_name_too_long(self):
        collector = BooksCollector()
        collector.add_new_book("Проверяем, что книга не добавляется, если её имя слишком длинное")
        assert "Проверяем, что книга не добавляется, если её имя слишком длинное" not in collector.get_books_genre()
